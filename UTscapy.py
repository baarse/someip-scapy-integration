...
# $Id: UTscapy.py,v 1.23 2006/07/18 23:11:48 pbi Exp $

import scapy,sys,getopt
...
def remove_empty_testsets(test_campaing):
    test_campaign.campaign = filter(lambda ts: len(ts.set) > 0, test_campaign.campaign)
...
""" % test_campaign
    ...
    if local:
        External_Files.UTscapy_js.write(os.path.dirname(OUTPUTFILE.name))
        External_Files.UTscapy_css.write(os.path.dirname(OUTPUTFILE.name))
...
if __name__ == "__main__":
    ...
    VERB=2
    try:
        opts = getopt.getopt(sys.argv[1:], "o:t:f:hln:m:k:K:DdCFq")
        for opt,optarg in opts[0]:
            if opt == "-h":
                ...
                r = imp.find_module(optarg)
                try:
                    mod = imp.load_module(optarg, *r)
                finally:
                    if r[0]:
                        r[0].close()
